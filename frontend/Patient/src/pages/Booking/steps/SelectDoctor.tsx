...
export interface DoctorResource {
    ...
    image: string;
    rating: number;
    clinicIds: string[]; // Added clinicIds
}
        ...
        <div className="space-y-8">
            <div className="flex flex-col items-center text-center mb-10">
                ...
                <p className="text-slate-400 max-w-md">
                    {clinicId ? "Experts available at this location." : "Select an expert practitioner to guide your personal wellness journey."}
                </p>
            </div>

            {filteredDoctors.length === 0 ? (
                <div className="text-center py-12 bg-slate-50 rounded-2xl border border-slate-100">
                    <p className="text-slate-500">No doctors available at this clinic based on current roster.</p>
                </div>
            ) : (
